...
class QuizGenerator:
    ...
    def generate_question_with_vectorstore(self):
        ...
        if not self.vectorstore.db:
            raise Exception("Failed to build vector DB.")
        ...
        ### Imaging you have two retrivers, thats when the parallel shine, ex. {"context_1": retriever_1, "context_2": retriever_2, "topic": RunnablePassthrough()}
        setup_and_retrieval = RunnableParallel(
            {"context": retriever, "topic": RunnablePassthrough()}  # The two variables in the prompt template!
